...
A Red–Black tree is a kind of self-balancing binary search tree. The main
difference to normal binary trees, is that they store an extra bit (the _color_)
in each node. This bit can be either be 0 (_black_) or 1 (_red_). This is
how a red-black tree looks like, conceptually:

```{r, out.width = "300px", echo=FALSE, fig.align = "center", fig.cap="A red-black tree"}
knitr::include_graphics("rb-tree.png")
```

The following invariant must be satisfied every time an operation is
being done on a node of a Red-Black tree:
...
The tree itself is nothing more that a collection of nodes starting from
a root node. The constructor initializes a root node to a provided value
...
Your task for this assignment is to constuct algorithms to manipulate Red-Black
trees.

## Constructing a Red-Black Tree
We start constructing an Red-Black tree by calling the constructor
```
tree = RBTree(5)
```
Inserting a node consists of three steps:

1. Finding the appropriate point to insert the node to
...
**T5 (30 points)**: Create a function that given a `RBTree` object it will exhaustively search
whether the five invariants described above are maintained.
