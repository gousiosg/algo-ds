...
Using this table, if we are search for `ABABA` in `ABABCABBAC`, we can
jump ahead to $T$ position $cur + i - jump[i] + 1$, where $i$ is the index of 
the character in $P$ that failed to match, _i.e._ restart at `C`
