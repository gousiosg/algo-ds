...
* `def remove:` Additional: Search for the successor before removing the element
...
* A tree with the same number of nodes on the left subtree and right subtree
...
* For every node $n$ in an AVL-tree, the heights of the left and
  right subtrees of $n$ can differ at most by 1.

## AVL-tree (2)

* The height of the tree should be bound by $\mathcal{O}(\log{}_2n)$ at worst
  case
* Let's consider the height of an AVL tree as $h: AVL_h$
* To satisfy the balance invariant:
    + The **root** must have a child with height $h-1$ with $AVL_{h-1}$ nodes
    + The other child must than have a height of $h-2$  with $AVL_{h-2}$ nodes
* The AVL tree has in total  $AVL_{h-1} + AVL_{h-2} + 1$ nodes (1 being the root
* itself)  

## AVL-tree (3)
* The base cases are $h=1,2$, the heights are $AVL_1=1, AVL_2=2$
* Iteratively, we can construct $AVL_h$ as $AVL_h = AVL_{h-1} + AVL_{h-2} + 1$ as follows
    + $AVL_{h-1}$ is left sub-tree and $AVL_{h-2}$  is the right sub-tree
    + $AVL_h = AVL_{h-1} + AVL_{h-2} + 1$ 
    + $AVL_{h-1} = AVL_{h-2} + AVL_{h-3} + 1$ 
    + $AVL_{h} = (AVL_{h-2} + AVL_{h-3} + 1)+ AVL_{h-2} + 1$ 
    + $AVL_{h} > 2AVL_{h-2}$ 
    + $log AVL_{h} > log 2AVL_{h-2}$ 
    + $2AVL_{h} > h := h = \mathcal{O}(\log{}_n)$  

## AVL operations

* We have shown that introduced invariant can maintain the tree balanced at
  $\mathcal{O}(\log{}_n)$ 
...
What ever we rotate, wont solve the imbalance

## Second Case: $\beta$ inserted in $B$ (1)
Make a left-rotation on $x$
![](./figures/tree_lr_example.svg){ width=80% }

## Second Case: $\beta$ inserted in $B$ (2)
Make a right-rotation on $y$
![](./figures/tree_double.svg){ width=80% }

This is called a double right rotation

left rotation and double left rotation is symmetric to this
