...
* A tree with the same number of nodes on the left subtree and right subtree
  have a *worst-case* complexity of $\mathcal{O}(\log{}_2n)$
...
* We can quickly see that the worst case is similar to traverse an entire list 
  and the complexity is $\mathcal{O}(n)$
* There are many scenarios where we end up with an unbalanced tree, this makes 
  the worst case scenario for BST operations to be $\mathcal{O}(n)$
...
* We want to avoid that a BST degenerates to $\mathcal{O}(n)$ time
