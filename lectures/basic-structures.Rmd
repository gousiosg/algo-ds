...
Like all containers, dynamic collections can grow, shrink, or change over time.

- Dynamic collections support basic operations such as:
...
## New Array Elements After the ```insert``` operation
...
## New Array Elements After the ```remove``` operation
...
class Set(object):
    ...
    def __init__(self, iterable):
        ...
        for elem in data[1:]:
            if elem == result[-1]:
                continue
            # XX: The append method has been implement in the previous slide
            result.append(elem)
        self.data = result
```
