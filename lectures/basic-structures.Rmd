...
def insert(arr, item, k):
	global n
	j = n - 1
	# add one more cell to the array
	n = n + 1
	arr = arr + [None]
...
def main():
	print(arr)
	# insert a new item before the 5th item of arr
	print(insert(arr, 10, 4))

if __name__ == "__main__":
    main()
```

## Array Elements After the ```insert``` Operation
...
## Example of the ```remove``` Operation
...
## Array Elements After the ```remove``` Operation
...
## Example of the ```search``` Operation
...
The item = 5 has index = 4.
```

## Stacks

- Stacks and queues are *abstract data types* that represent *dynamic collections*,
where the way an element can be removed from a set is prespecified (*LIFO* or *FIFO*).
...
new elements 17 and 3 are added?
```{r echo=FALSE}
...
def isEmpty(stack, n):
	...
	j = 0
	while( j < n):
		if((stack[j] == stack[n-1]) and (stack[j] is None)):
			res = j
			break
...
## Example of the ```isEmpty``` Operation

```python
def main():
	stack = [None] * 8;
	maxsize = 8
	if (isEmpty(stack, maxsize) == 0):
		print("Empty stack")
...
## Example of the ```isTop``` Operation
...
```python
def push(stack, item, n):
	# index of the top element in the stack
	top = isTop(stack, n)
	# ensure that the stack is not full
	if(top != n - 1):
		top = top + 1
		stack[top] = item
	else:
...
- If a **queue** consists of elements ```Q[1..n]```, ```Q.head``` points to its *head* and ```Q.tail``` points the *next* position where a new element can be added to the queue. When ```Q.head = Q.tail```, it means that the queue is *empty*.
...
def main():
	...
	enqueue(4)
	enqueue(5)
	print(str(arr) + " tail: " + str(tail))
		
if __name__ == "__main__":
    main()
    
# Output:
# [3, 4, 5, None, None, None] tail: 2
```
...
# 3
# 4
# [3, 4, None, None, None, None] tail: -1
# True
```
