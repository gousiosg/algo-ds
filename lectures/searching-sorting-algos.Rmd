...
2. Swap $C[0]$ with $C[x]$ where $x$ is the index of the min element
3. Traverse to the next element, $C[1]$
4. Repeat step 1 and 2 until the end of the list
...
* The previous methods focus on exchanging elements with neighbouring elements
* For faster sorting, we could increase the distance and not limit to exchanging 
  with neighbouring elements 
* There are two algorithms for faster sorting:
    + Quicksort: selection of a *pivot* value, 
    + Mergesort: split until we have array of length 1, and merge pair of values
      until we have one list
...
1. Select a *pivot*
    + simplest: select first element
    + another: select the middle value of three randomly selected values
    + another: the middle value of the list
2. Add a pointer on the first element and the last element
3. Move the *first* element pointer to a value that is $>$ pivot
4. Move the *last* element pointer to a value that is $\le$ pivot
5. Swap theses values and continue recursively with sort *element* $\le$ pivot and
   *element* $>$ pivot
...
def qs(lst, left, right):
    if right - left > 1:
        ...
        ri = right
        while le <= ri:
            while le <= right and lst[le] < pivot:
                le += 1
            while ri > left and lst[ri] >= pivot:
                ri -= 1
            if le < ri:
                lst[le], lst[ri] = lst[ri], lst[le]
        lst[ri], lst[left] = lst[left], lst[ri]
        qs(lst, left, ri-1)
        qs(lst, ri+1, right
```
...
Merge sort is useful for large lists or files. The idea is the following:

1. Compare the first element in each list and take the smallest value until one
   of the lists are done
2. Continue on non-empty lists
...
* The shown algorithms are not optimized
    + Merge sort could have used index instead of creating arrays
    + Quick sort: use a stack instead of recursion
* While both are  $O(n log n)$, what is a very distinct complexity aspect to
  consider?
